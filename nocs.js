...
var vm = function () {
    ...
    self.search = function () {
        ...
        ajaxHelper(searchUrl, 'GET').done(function (data) {
            ...
            var enrichedData = data.filter(function (noc) {
                return noc.Country.toLowerCase().includes(searchQuery); // Alterado para "includes"
            }).map(function (noc) {
                return {
                    Id: noc.Id,
                    Country: noc.Country,
                    Athletes: noc.Athletes,
                    Coaches: noc.Coaches,
                    Medals: noc.Medals,
                    Teams: noc.Teams,
                    Photo: noc.Photo
        ...
        source: function (request, response) {
            $.ajax({
                ...
                success: function (data) {
                    var nocCountries = data.map(function (noc) {
                        return noc.Country;
                    });

                    // Alteração: Use "includes" para verificar se o termo está contido em qualquer parte do nome
                    response($.grep(nocCountries, function (country) {
                        return country.toLowerCase().includes(request.term.toLowerCase());
                    }));
                }
            });
        },
