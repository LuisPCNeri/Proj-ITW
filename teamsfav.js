...
function removeFav(teamId) {
    ...
    favourites = favourites.filter(fav => fav.team !== teamId);
...
$(document).ready(function () {
    ...
    for (const fav of favourites) {
        ...
        if (fav.team) {
            console.log("Carregando equipa favorita com ID:", fav.team);

            // Fazer a chamada à API com o ID associado à chave `team`
            ajaxHelper(`http://192.168.160.58/Paris2024/api/Teams/${fav.team}`, 'GET').done(function (data) {
                ...
                $("#table-favourites").append(
                    `<tr id="fav-${fav.team}">
                        <td class="align-middle">${fav.team}</td>
                        <td class="align-middle">${data.Name}</td>
                        <td class="align-middle">${data.Sex}</td>
                        <td class="align-middle">${data.Country}</td>
                        <td class="text-end align-middle">
                            <a class="btn btn-default btn-sm btn-favourite" onclick="removeFav('${fav.team}')">
