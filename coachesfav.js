...
function activatePage(page) {
    ...
    var composedUri = `http://192.168.160.58/Paris2024/api/Coaches?page=${page}&pageSize=20`;
...
function removeFav(teamId) {
    ...
    favourites = favourites.filter(fav => fav.coach !== teamId);
...
$(document).ready(function () {
    ...
    for (const fav of favourites) {
        ...
        if (fav.coach) {
            console.log("Carregando equipa favorita com ID:", fav.coach);

            // Fazer a chamada à API com o ID associado à chave `team`
            ajaxHelper(`http://192.168.160.58/Paris2024/api/Coaches/${fav.coach}`, 'GET').done(function (data) {
                ...
                $("#table-favourites").append(
                    `<tr id="fav-${fav.coach}">
                        <td class="align-middle">${fav.coach}</td>
                        <td class="align-middle">${data.Name}</td>
                        <td class="align-middle">${data.Function}</td>
                        <td class="align-middle">${data.Sex}</td>
                        <td class="text-end align-middle">
                            <a class="btn btn-default btn-sm btn-favourite" onclick="removeFav(${fav.coach})">
